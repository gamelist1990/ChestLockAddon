...
registerCommand({
    ...
    maxArgs: 2,
    minArgs: 1,
    require: (player) => verifier(player, c().commands["chest"]),
...
function sendInvalidCommandMessage(player) {
...
// 近くのチェストの情報を表示する関数
function showNearbyChestInfo(player) {
...
// チェストを保護する関数
function protectChest(player, lockState) {
    const nearbyChestLocation = findNearbyChest(player);
    if (nearbyChestLocation) {
        const chestKey = getChestKey(nearbyChestLocation);
        const chestData = protectedChests[chestKey];
...
function findNearbyChest(player) {
...
export function showProtectedChestData(player) {
...
export function resetProtectedChests(player) {
    protectedChests = {};
    saveProtectedChests();
...
function handlePistonUse(eventData) {
    let playerActions = {};
    ...
    if (RADIUS2_IDS.includes(itemId)) {
        if (!isWithinDetectionArea(blockLocation, RADIUS2))
            return;
    }
    else if (RADIUS1_IDS.includes(itemId)) {
        if (!isWithinDetectionArea(blockLocation, RADIUS1))
            return;
    }
    else {
        return;
...
// 検知範囲内かどうか判定
function isWithinDetectionArea(location, radius) {
...
function isMovingTowardsChest(locations) {
...
// 保護されたチェストかどうか判定
function isProtectedChest(location) {
    const key = `${location.x},${location.y},${location.z}`;
    return protectedChests.hasOwnProperty(key);
}
async function revertChest(location) {
    const key = `${location.x},${location.y},${location.z}`;
...
function handleChestBreak(event) {
    const chestKey = getChestKey(event.block.location);
    const chestData = protectedChests[chestKey];
    if (chestData) {
        if (event.player) {
...
// 爆発によるチェスト破壊を処理する関数
function handleExplosion(eventData) {
...
function findAdjacentChest(location) {
    const directions = [
        { x: 1, y: 0, z: 0 },
        { x: -1, y: 0, z: 0 },
        { x: 0, y: 0, z: 1 },
...
// チェストの保護状態を切り替える関数
function toggleChestProtection(player, state) {
    const nearbyChestLocation = findNearbyChest(player);
    if (nearbyChestLocation) {
        const chestKey = getChestKey(nearbyChestLocation);
        const chestData = protectedChests[chestKey];
...
// メンバー追加関数
function addMember(player, memberName) {
    const nearbyChestLocation = findNearbyChest(player);
    if (nearbyChestLocation) {
        const chestKey = getChestKey(nearbyChestLocation);
        const chestData = protectedChests[chestKey];
...
// メンバー削除関数
function removeMember(player, memberName) {
    const nearbyChestLocation = findNearbyChest(player);
    if (nearbyChestLocation) {
        const chestKey = getChestKey(nearbyChestLocation);
        const chestData = protectedChests[chestKey];
