...
function sendPlayerInfoToChat(player: Player, targetPlayer: Player): void {
  ...
  if (!targetPlayerData) {
    player.sendMessage(translate(player, "commands.list.playerNotFound", { tragetplayer: `${targetPlayer.name}` }));
    return;
  }

  const healthComponent = targetPlayerData.getComponent("minecraft:health") as EntityHealthComponent;
  ...
  player.sendMessage(translate(player, "commands.list.playerInfo", {
