...
function showHotbarItemActions(player: Player, targetPlayer: Player, selectedItemIndex: number, playerInv: any) {
    const targetInv = targetPlayer.getComponent('inventory')?.container;
        ...
        if (result.selection === 0) {
            ...
            if (playerInv && targetEquipment) {
                const equipmentSlot: EquipmentSlot =
                    selectedItemIndex === 0 ? EquipmentSlot.Head :
                        selectedItemIndex === 1 ? EquipmentSlot.Chest :
                            selectedItemIndex === 2 ? EquipmentSlot.Legs :
                                EquipmentSlot.Feet;
...
function showInventory(player: Player, targetPlayer: Player, currentPage = 0) {
    ...
    const form = new ActionFormData()
        .title(`§4${targetPlayer.nameTag}'s Inventory (${currentPage + 1}/${totalPages})§4`);
