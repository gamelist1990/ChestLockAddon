...
// 掘削処理を行う関数
function runDrillAction(player: Player, drillItem: any, options: DrillOptions) {
...
consoles.then((player: Player) => {
    system.run(() => {
        const playerLocation = player.location;
        const dimension = player.dimension;
        const RADIUS = 5; // 効果範囲
        const TELEPORT_RADIUS = 2; // テレポート範囲　最大半径
        const MIN_TELEPORT_RADIUS = 1; //テレポート最小範囲

        const DURATION = 20; // 効果時間 (秒)
        const INTERVAL = 20 * 2; // テレポート間隔 (tick) 1秒
        ...
        function getRandomCoordinate(
            ...
            do {
                ...
                coord = center + (Math.random() * 2 - 1) * radius;
        ...
        if (nearbyPlayers.length > 0) {
            const targetPlayer =
                nearbyPlayers[Math.floor(Math.random() * nearbyPlayers.length)];
            ...
            teleportIntervalId = system.runInterval(() => {
                ...
                if (ticks < DURATION * (20 / INTERVAL)) {


                    let randomX = getRandomCoordinate(
                        targetPlayer.location.x,
                        MIN_TELEPORT_RADIUS,
                        TELEPORT_RADIUS,
                        MIN_X,
                        MAX_X
            ...
            consoles.removeItem(player, consoles.get());
        } else {
            ...
            try {
                switch (itemName) {
                    ...
                    case "barrier":
                        ...
                        break;
                    case "drill":
                        drillItem.give(player, amount);
                        player.sendMessage(`§a${drillItem.name} §7を ${amount} 個入手しました`);
                        break;
                    case "consoles":
                        consoles.give(player, amount);
                        player.sendMessage(`§a${consoles.name} §7を ${amount} 個入手しました`);
                        break;
                    default:
