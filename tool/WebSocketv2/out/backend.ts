...
export class WsServer {
  ...
  private mergeAndCleanPlayerData(
  ...
  private rapidJoinLeaveTimeout: number = 100;
  ...
  private async checkOnlineStatus() {
    try {
      ...
      if (listResult?.statusCode === 0 && listResult?.players && listResult?.statusMessage) {
        ...
        for (const playerName of this.onlinePlayerNamesCache) {
          if (!this.activePlayers.has(playerName)) {
            ...
            if (player) {
              //playerがnullの場合のチェック
              // 短期間での再入室チェック
              const lastLeftTime = this.lastLeftTimes.get(player.uuid);
              const now = Date.now();

              if (lastLeftTime && now - lastLeftTime < this.rapidJoinLeaveTimeout) {
                console.warn(
                  `[Rapid Join/Leave Detected] Player ${playerName} (UUID: ${player.uuid}) attempted to rejoin too quickly. Ignoring.`,
                );
                continue; // 参加処理をスキップ
              }
              ...
              if (playerIndex === -1) {
                ...
                // existingPlayer.uuid = player.uuid; // UUIDは変わらないので更新不要
        ...
        for (const [playerName, player] of this.activePlayers) {
          if (!this.onlinePlayerNamesCache.includes(playerName)) {
            ...
            // 最終退出時刻を記録
            this.lastLeftTimes.set(player.uuid, Date.now());
          }
        }
      } else {
      ...
      for (const player of playerData) {
        ...
        if (player.uuid) {
          const onlinePlayer = Array.from(this.activePlayers.values()).find(
          ...
          if (player.isOnline !== isOnline) {
            ...
            console.log(
              `Data fixed: Player ${player.name} isOnline status corrected to ${isOnline}.`,
            );
          }
          // 名前が一致しない場合も修正
...
export function removeCommand(name: string) {
  if (typeof wsserver !== 'undefined' && wsserver.commands && wsserver.commands[name]) {
    ...
    console.warn(
      `[CommandManager] Command '${name}' not found or wsserver.commands is not accessible.`,
    );
  }
}
