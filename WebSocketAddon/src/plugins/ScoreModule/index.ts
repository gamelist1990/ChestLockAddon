import { world, system, ScoreboardIdentity, ScoreboardObjective, Player } from '@minecraft/server';
...
§r  - §9[true]§r: 最大人数チーム。無指定で0人チーム,同数はsame`;
    ...
    registerCommands(handler: Handler): void {
        ...
        handler.registerCommand("team", {
            ...
            execute: (message, event) => {
                ...
                if (args.length === 0) {
                    sendMessage("使用方法: /ws:team set <チーム数>:<チーム内上限人数> <タグ名> <スコアボードタイトル>");
                    return;
                }
                ...
                if (subcommand === "set") {
                    if (args.length < 4) {
                        sendMessage("引数が不足しています。使用方法: /ws:team set <チーム数>:<チーム内上限人数> <タグ名> <スコアボードタイトル>");
                        return;
                    }
        ...
        handler.registerCommand("teamCount", {
            ...
            execute: (message, event) => {
                ...
                try {
                    ...
                    if (args.length < 2) {
                        consoleOutput("使用方法: /ws:teamCount <チームタグ1,チームタグ2,...> <コマンドリスト> [true]");
                        return;
                    }
        ...
        async function showDeleteForm(player: Player) {
            ...
            const wsObjectives = world.scoreboard.getObjectives()
     ...
     */
        async function confirmAndDeleteAll(player: Player) {
            ...
            confirmForm.body("本当に ws_module 以外の ws_ から始まるすべてのスコアボードを削除しますか？");
            confirmForm.button1("はい");
            confirmForm.button2("いいえ");
            ...
            if (confirmResponse.selection === 0) {
                const objectivesToRemove = world.scoreboard.getObjectives()
    ...
    // スコアボードをリセットする共通の処理
    private resetScoreboard(objective: ScoreboardObjective, sendMessage: (message: string) => void): void {
        for (const participant of objective.getParticipants()) {
            try {
                objective.removeParticipant(participant);
            } catch (error) {
                console.error(`Error removing participant ${this.getParticipantDisplayName(participant)} from ${objective.displayName}: ${error}`);
                sendMessage(`Error removing participant ${this.getParticipantDisplayName(participant)} from ${objective.displayName}`);
            }
        }
    }
...
function resolvePlayerName(key: string): string {
    ...
    for (const [pattern, replacement] of Object.entries(simpleReplacements)) {
        ...
        playerNameResolved = playerNameResolved.replace(regex, typeof replacement === 'function' ? replacement() : replacement);
    }
    ...
    // score の置換
    playerNameResolved = playerNameResolved.replace(/\[score=([^,]+)(?:,([^\]]+))?\]/g, (_, scoreTitle, playerName) => {
